...
WORKDIR /quarkus
COPY --from=maven /quarkus/hibernate-event-loop/target/lib lib
COPY --from=maven /quarkus/hibernate-event-loop/target/hibernate-event-loop-1.0-SNAPSHOT-runner.jar app.jar
